...
export async function generateMetadata({ params }: { params: Promise<{ region: string }> }): Promise<Metadata> {
  ...
  const title = `Dārza kalendārs ${r.name} — salnas, sējas logi un Mēness ritms`;
  const description = `Kad sēt un stādīt ${r.name} (${r.climateLabel}): vidējā pēdējā salna ${r.lastFrost}, ${r.growingDays} augšanas dienas. Reģionam pielāgots Mēness sējas kalendārs.`;
...
export default async function RegionPage({ params }: { params: Promise<{ region: string }> }) {
  ...
  const jsonLd = {
    ...
    "@type": "Article",
    headline: `Dārza kalendārs ${r.name}`,
    inLanguage: "lv",
  ...
  return (
    <article className="mx-auto max-w-3xl">
      ...
      <header className="mb-lg">
        ...
        <h1 className="text-headline-lg-mobile text-primary md:text-display-lg">Dārza kalendārs {r.name}</h1>
      ...
      <Card tone="highest" elevated accent="primary" className="mb-md p-md">
        <h2 className="mb-1 text-headline-md text-on-surface">Sējas padoms {r.name}</h2>
        <p className="text-body-md text-on-surface-variant">
          Siltummīļus (tomāti, gurķi, paprika) {r.name} laukā stādi tikai pēc vidējās pēdējās
